(function() {
  ...
  let capsuleScaleTarget = 1;
  let capsuleX = 0;
  let capsuleW = 0;
  let capsuleSettleLockUntil = 0;
  ...
  function updateGlobalAvatarPositionNow() {
    ...
    // home(0) → blog(1) → works(2) → playlist(3) → contact(4)
    if (p > 4) p = 4;
  ...
  function tickCapsuleScaleSpring() {
    const target = capsuleScaleTarget;
  ...
  function setCapsuleScaleTarget(next, immediate) {
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
      capsuleScaleTarget = 1;
      capsuleScale = 1;
      capsuleScaleVel = 0;
      applyCapsuleTransform();
      return;
    }
    if (!navCapsule || (nav && nav.classList.contains('blog-mode'))) {
      capsuleScaleTarget = 1;
      return;
    }
    capsuleScaleTarget = next;
    if (immediate) {
      capsuleScale = next;
      capsuleScaleVel = 0;
      applyCapsuleTransform();
      if (capsuleScaleRaf) {
        cancelAnimationFrame(capsuleScaleRaf);
        capsuleScaleRaf = null;
      }
      return;
    }
  ...
  function updateCapsuleFromScroll() {
    if (capsuleDragging) return;
    if (performance.now() < capsuleSettleLockUntil) return;
    ensureNavCapsule();
    if (!navCapsule) return;
  ...
  function setupNavCapsuleDrag() {
    ...
    function beginDrag(e) {
      ...
      setCapsuleScaleTarget(CAPSULE_SCALE_MAX, false);
    ...
    function endDrag(e) {
      ...
      setCapsuleScaleTarget(1, false);
      try { if (e) links.releasePointerCapture(e.pointerId); } catch (_) {}
      ...
      applyProgressToCapsuleAndScroll(nearest, false);
      lastDragProgress = nearest;

      const sectionId = btns[nearest] && btns[nearest].getAttribute('data-section');
      if (sectionId) ensureRouteLoaded(sectionId);
      const target = sectionId ? document.getElementById(sectionId) : null;
      capsuleSettleLockUntil = performance.now() + 480;
      if (target) {
  ...
  function renderProfile() {
    ...
    if (fullAvatarUrl) {
      ...
      globalAvatar.innerHTML = `<i class="" style="font-size: 4rem; color: #ffffff;"></i>`;
    }
  ...
  async function ensureRouteLoaded(sectionId) {
    ...
    if (id === 'playlist') return loadPlaylistRoute();
  ...
  function amTrackKey(t) {
    if (!t) return '';
    return String(t.id || t.src || t.audio || t.title || '');
  }
  
  function amRebuildShuffleOrder(preferKey) {
    const n = amState.tracks.length;
    const order = Array.from({ length: n }, (_, i) => i);
    for (let i = n - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      const t = order[i]; order[i] = order[j]; order[j] = t;
    }
    amState.shuffleOrder = order;
    if (preferKey) {
      const idx = amState.tracks.findIndex((t) => amTrackKey(t) === preferKey);
      if (idx >= 0) {
        ...
        amState.shufflePos = 0;
        return;
      }
    }
    amState.shufflePos = order.length ? 0 : -1;
  }

  function amEnsureShuffleOrder() {
    if (!Array.isArray(amState.shuffleOrder) || amState.shuffleOrder.length !== amState.tracks.length) {
      const cur = amGetPlayingTrack();
      amRebuildShuffleOrder(cur ? amTrackKey(cur) : '');
    }
  }

  function amGetPlayingTrack() {
    if (amState.currentTrack) return amState.currentTrack;
    if (amState.index >= 0 && amState.tracks[amState.index]) return amState.tracks[amState.index];
    return null;
  }
  function amFindIndexInTracks(track) {
    if (!track) return -1;
    const k = amTrackKey(track);
    return amState.tracks.findIndex((t) => amTrackKey(t) === k);
  }
  function amUpdateHeroForContext() {
    const heroTitle = document.getElementById('amHeroTitle');
    const heroSub = document.getElementById('amHeroSub');
    const heroArt = document.getElementById('amHeroArt');
    const kicker = document.querySelector('.am-hero-kicker');
    const playing = amGetPlayingTrack();
    const inList = playing ? amFindIndexInTracks(playing) >= 0 : false;
    if (amState.playerMode && playing) {
      if (heroTitle) heroTitle.textContent = playing.title || '未命名歌曲';
      if (heroSub) heroSub.textContent = (playing.artist && String(playing.artist).trim()) ? playing.artist : '—';
      if (kicker) kicker.textContent = '正在播放';
      amSetCover(heroArt, playing.cover || '');
      return;
    }
    if (inList && playing) {
      if (heroTitle) heroTitle.textContent = playing.title || '未命名歌曲';
    ...
    if (amState.view === 'library') {
      if (kicker) kicker.textContent = '资料库';
      if (heroTitle) heroTitle.textContent = '资料库';
      if (heroSub) heroSub.textContent = (amState.tracks.length || 0) + ' 首曲目 · 点击歌曲开始播放';
      amSetCover(heroArt, '');
    } else {
      const pl = amState.playlists.find((p) => p.id === amState.activePlaylistId);
      if (kicker) kicker.textContent = '播放列表';
      if (heroTitle) heroTitle.textContent = pl ? pl.name : '歌单';
      if (heroSub) heroSub.textContent = (pl ? pl.songs.length : 0) + ' 首曲目 · 点击歌曲开始播放';
      const withCover = ((pl && pl.songs) || []).filter((s) => s && s.cover);
      if (withCover.length) {
        const pick = withCover[Math.floor(Math.random() * withCover.length)];
        amState.heroPickTrack = pick;
        amSetCover(heroArt, pick.cover);
      } else {
        amState.heroPickTrack = (pl && pl.songs && pl.songs[0]) || null;
        amSetCover(heroArt, '');
      }
    }
  }
  function amCyclePlayMode() {
    const order = ['shuffle', 'order', 'loop'];
    const i = order.indexOf(amState.playMode || 'shuffle');
    amState.playMode = order[(i + 1) % order.length];
    if (amState.playMode === 'shuffle') {
      amRebuildShuffleOrder(amGetPlayingTrack() ? amTrackKey(amGetPlayingTrack()) : '');
    }
    amSyncPlayModeUi();
  }
  function amSyncPlayModeUi() {
    const btn = document.getElementById('amPlayModeBtn');
    if (!btn) return;
...
function amSetLyricsOpen(on) {
  ...
  const AM_CATALOG_URLS = [
    ...
    'https://raw.githubusercontent.com/Suixiuliang/Suixiuliang.github.io/master/playlist.json',
  ...
  function amAudioUrl(val) {
    if (typeof val === 'string') return /^(https?:\/\/|\/)/i.test(val) ? val : '';
    return amPick(val, AM_AUDIO_KEYS);
  }
  function amFileNameTitle(src) {
    try {
      const u = new URL(src, location.href);
      const n = decodeURIComponent(u.pathname.split('/').pop() || '').replace(/\.[^.]+$/, '');
      return n || '未命名';
    } catch (_) { return String(src || '').split('/').pop().replace(/\.[^.]+$/, '') || '未命名'; }
  }
  function amLooksLikeSong(val) { return !!amAudioUrl(val); }
  function amParseSong(key, val, playlistName, idx) {
    const src = amAudioUrl(val);
    if (!src) return null;
    const id = String(playlistName || 'pl') + '-' + idx + '-' + String(key || 't');
    const isObj = val && typeof val === 'object';
    let title = typeof val === 'string' ? String(key || amFileNameTitle(src)) : amPick(val, AM_TITLE_KEYS);
    if (!title || title === String(key || '')) title = String(key || amFileNameTitle(src));
    const artist = isObj ? (amPick(val, ['artist', 'artists', '歌手', '演唱', 'author']) || '') : '';
    const album = isObj ? (amPick(val, ['album', '专辑', 'albumName']) || '') : '';
    const year = isObj ? (amPick(val, ['year', 'date', '年份']) || '') : '';
    // 封面 / LRC：只认 playlist 配置，不读音文件内嵌标签
    const cover = isObj ? (amPick(val, ['cover', 'coverUrl', 'artwork', 'pic', 'image', '封面', '封面图']) || '') : '';
    const lyrics = isObj ? (amPick(val, ['lyrics', 'lrc', 'lyric', 'lyricsUrl', 'lrcUrl', '歌词']) || '') : '';
    return {
      id, title, artist, album, year, cover, lyrics, src,
      duration: 0, metadataLoaded: true, metadataLoading: false
    };
  }
  function amParseSongBag(bag, playlistName) {
    const songs=[]; if (!bag) return songs;
    if (Array.isArray(bag)) { bag.forEach((item,i)=>{ const s=amParseSong(item && (item.title||item.name||item['歌名']) || ('曲目 '+(i+1)), item, playlistName, i); if(s)songs.push(s); }); return songs; }
    if (typeof bag==='object') { let i=0; Object.keys(bag).forEach(k=>{ const s=amParseSong(k,bag[k],playlistName,i++); if(s)songs.push(s); }); }
    return songs;
  }
  function amParseCatalog(data) {
    if (!data) return [];
    if (typeof data==='string') { try{data=JSON.parse(data)}catch(_){return []} }
    if (data.playlists) return amParseCatalog(data.playlists);
    if (data.歌单) return amParseCatalog(data.歌单);
    const out=[];
    if (Array.isArray(data)) {
      data.forEach((pl,i)=>{
        if(!pl)return;
        if(amLooksLikeSong(pl)) { const s=amParseSong(pl.title||pl.name||('曲目 '+(i+1)),pl,'默认歌单',i); if(s){let g=out.find(x=>x.id==='default'); if(!g){g={id:'default',name:'默认歌单',songs:[]};out.push(g)} g.songs.push(s)} return; }
        const name=String(pl.name||pl.title||pl['名称']||pl['歌单']||('歌单 '+(i+1)));
        const bag=pl.songs||pl.tracks||pl.items||pl['曲目']||pl['歌曲'];
        const songs=amParseSongBag(bag,name); if(songs.length)out.push({id:'pl-'+i,name,songs});
      });
      return out;
    }
    if(typeof data==='object') Object.keys(data).forEach((k,i)=>{
      if(k==='meta'||k==='version'||k==='updated')return;
      const val=data[k];
      if(amLooksLikeSong(val)){const s=amParseSong(k,val,'默认歌单',i); if(s){let g=out.find(x=>x.id==='default');if(!g){g={id:'default',name:'默认歌单',songs:[]};out.push(g)}g.songs.push(s)}return;}
      const name=(val&&typeof val==='object'&&(val.name||val.title||val['名称']))?String(val.name||val.title||val['名称']):k;
      const bag=(val&&typeof val==='object'&&(val.songs||val.tracks||val.items||val['曲目']||val['歌曲']))?(val.songs||val.tracks||val.items||val['曲目']||val['歌曲']):val;
      const songs=amParseSongBag(bag,name); if(songs.length)out.push({id:'pl-'+i,name,songs});
    });
    return out;
  }
  ...
  function amTrimText(s){ return String(s||'').replace(/\u0000/g,'').replace(/^\s+|\s+$/g,''); }
  function amSynchsafe(a,b,c,d){ return ((a&0x7f)<<21)|((b&0x7f)<<14)|((c&0x7f)<<7)|(d&0x7f); }
  function amU32(a,b,c,d){ return (((a<<24)>>>0)|(b<<16)|(c<<8)|d)>>>0; }
  function amParseId3(buf) {
    const a=new Uint8Array(buf), out={artist:'',album:'',year:'',coverBlob:null,lyrics:''};
    if(a.length<10 || String.fromCharCode(a[0],a[1],a[2])!=='ID3') return out;
    const ver=a[3], flags=a[5], tagSize=amSynchsafe(a[6],a[7],a[8],a[9]);
    let pos=10, end=Math.min(a.length,10+tagSize);
    if(flags&0x40){ if(ver>=4 && pos+4<=end){const x=amSynchsafe(a[pos],a[pos+1],a[pos+2],a[pos+3]);pos+=x;} else if(pos+4<=end){const x=amU32(a[pos],a[pos+1],a[pos+2],a[pos+3]);pos+=x;} }
    while(pos<end){
      let id='', size=0, head=0;
      if(ver===2){ if(pos+6>end)break; id=String.fromCharCode(a[pos],a[pos+1],a[pos+2]); size=(a[pos+3]<<16)|(a[pos+4]<<8)|a[pos+5]; head=6; }
      else { if(pos+10>end)break; id=String.fromCharCode(a[pos],a[pos+1],a[pos+2],a[pos+3]); size=ver>=4?amSynchsafe(a[pos+4],a[pos+5],a[pos+6],a[pos+7]):amU32(a[pos+4],a[pos+5],a[pos+6],a[pos+7]); head=10; }
      if(!id || !size){pos+=head;continue;} if(pos+head+size>end)break;
      const d=a.subarray(pos+head,pos+head+size);
      if(/^(TPE1|TPE2|TP1|TP2)$/.test(id)) out.artist=amTrimText(amDecodeText(d,d[0])).replace(/\0.*$/,'');
      else if(/^(TALB|TAL)$/.test(id)) out.album=amTrimText(amDecodeText(d,d[0])).replace(/\0.*$/,'');
      else if(/^(TDRC|TYER|TDOR|TYE|TOR)$/.test(id)) out.year=amTrimText(amDecodeText(d,d[0])).replace(/\0.*$/,'').slice(0,4);
      else if(id==='USLT' || id==='ULT') {
        const enc=d[0]; let q=4; while(q<d.length && d[q]!==0)q++; q++;
        if(enc===1||enc===2){ if(q<d.length && d[q]===0)q++; else {while(q+1<d.length && !(d[q]===0&&d[q+1]===0))q+=2;q+=2;} }
        out.lyrics=amTrimText(amDecodeText(d.subarray(q),enc));
      } else if(id==='SYLT') {
        const enc=d[0]; let q=1; q+=3; q+=2; while(q<d.length&&d[q]!==0)q++; q++;
        if(enc===1||enc===2){while(q+1<d.length&&!(d[q]===0&&d[q+1]===0))q+=2;q+=2;}
        const lines=[]; while(q<d.length){
          const st=q; if(enc===0||enc===3){while(q<d.length&&d[q]!==0)q++;} else {while(q+1<d.length&&!(d[q]===0&&d[q+1]===0))q+=2;}
          let text=amTrimText(amDecodeText(d.subarray(st,q),enc)); q+=(enc===0||enc===3)?1:2; if(q+4>d.length)break; const t=amU32(d[q],d[q+1],d[q+2],d[q+3]); q+=4; if(text)lines.push('['+formatAmTime(t/1000)+'.'+String(t%1000).padStart(3,'0')+']'+text);
        }
        if(lines.length)out.lyrics=lines.join('\n');
      } else if(id==='APIC' || id==='PIC') {
        let q=0, enc=d[q++], mime='image/jpeg';
        if(id==='PIC'){ const fmt=String.fromCharCode(d[q++],d[q++],d[q++]); mime=fmt==='PNG'?'image/png':fmt==='GIF'?'image/gif':'image/jpeg'; }
        else { const st=q; while(q<d.length&&d[q]!==0)q++; mime=amTrimText(amDecodeText(d.subarray(st,q),0))||mime; q++; }
        q++; // picture type
        if(enc===1||enc===2){while(q+1<d.length&&!(d[q]===0&&d[q+1]===0))q+=2;q+=2;}else{while(q<d.length&&d[q]!==0)q++;q++;}
        if(q<d.length)out.coverBlob=new Blob([d.subarray(q)],{type:mime});
      }
      pos+=head+size;
    }
    return out;
  ...
  async function amLoadTrackMetadata(track) {
    // 不再读取音文件内嵌标签；封面与歌词只来自 playlist.json 的 cover / lyrics(lrc)
    if (!track) return;
  ...
  function amSelectPlaylist(id) {
    const pl = amState.playlists.find((p) => p.id === id) || amState.playlists[0];
    if (!pl) {
    ...
    amApplySortToTracks();
    // 若正在播放的歌在本歌单内，对齐 index
    const playing = amGetPlayingTrack();
    if (playing) {
      const idx = amFindIndexInTracks(playing);
      amState.index = idx;
    } else {
      amState.index = -1;
    }
  ...
  function renderAmPlaylistNav() {
    const box = document.getElementById('amPlaylistList');
    if (!box) return;
    if (!amState.playlists.length) {
      box.innerHTML = '<div class="am-playlist-list-label">播放列表</div><div class="am-playlist-loading">暂无歌单。请在仓库根目录放置 playlist.json</div>';
      return;
    }
    ...
    box.querySelectorAll('.am-playlist-item').forEach((btn) => {
      btn.addEventListener('click', () => amSelectPlaylist(btn.getAttribute('data-playlist-id')));
    });
  }
  ...
  function amApplySortToTracks() {
    const key = amState.sortKey || 'title';
    const dir = amState.sortDir || 'asc';
    const map = { title: 'title', artist: 'artist', album: 'album', playlist: 'album', year: 'year' };
    const field = map[key] || 'title';
    amState.tracks = (amState.tracks || []).slice().sort((a, b) => amCompareTracks(a, b, field, dir));
  }

  function amSetSort(key) {
    if (!key) return;
    if (amState.sortKey === key) {
      amState.sortDir = amState.sortDir === 'asc' ? 'desc' : 'asc';
    } else {
      amState.sortKey = key;
      amState.sortDir = 'asc';
    }
    const curId = amState.index >= 0 && amState.tracks[amState.index] ? amState.tracks[amState.index].id : null;
    amApplySortToTracks();
    if (curId) {
      const ni = amState.tracks.findIndex((t) => t.id === curId);
      if (ni >= 0) amState.index = ni;
    }
    renderAmTracks(document.getElementById('amSearchInput')?.value || '');
    amUpdateSortHeaders();
  }

  function amUpdateSortHeaders() {
    document.querySelectorAll('.am-track-head [data-sort]').forEach((el) => {
      const k = el.getAttribute('data-sort');
      el.classList.toggle('is-sorted', k === amState.sortKey);
      el.classList.toggle('is-asc', k === amState.sortKey && amState.sortDir === 'asc');
      el.classList.toggle('is-desc', k === amState.sortKey && amState.sortDir === 'desc');
      el.setAttribute('aria-sort', k === amState.sortKey ? (amState.sortDir === 'asc' ? 'ascending' : 'descending') : 'none');
    });
  }
  ...
  function renderAmTracks(filter) {
    ...
    list.innerHTML = rows.map((t, i) => {
      const playing = (typeof amGetPlayingTrack === 'function' ? amGetPlayingTrack() : null) || (amState.index >= 0 ? amState.tracks[amState.index] : null);
      const active = !!(playing && ((playing.id && playing.id === t.id) || (playing.src && playing.src === t.src)));
  ...
  function amSyncNowPlaying() {
    try { amUpdateLocateBtn(); } catch (_) {}
    // 英雄区「播放」在正在播放且显示当前曲时改为暂停
    try {
      const playAllBtn = document.getElementById('amPlayAllBtn');
      if (playAllBtn) {
        const playing = !!amState.playing && !!amGetPlayingTrack();
    ...
    const meta = document.getElementById('amNpMeta');
    const toggle = document.getElementById('amToggleBtn');
    const track = amGetPlayingTrack();
    if (!bar) return;
    if (!track) {
      bar.hidden = true;
      return;
  ...
  function amFixMojibake(text) {
    let t = String(text || '');
    if (!t) return t;
    // UTF-8 被当成 Latin-1 读出的典型乱码：â€™ Ã© etc.
    if (/â.|Ã.|Â.|å.|æ.|ç.|è.|é.|ï.|ð./.test(t)) {
      try {
        const bytes = Uint8Array.from(t, (ch) => ch.charCodeAt(0) & 0xff);
        const fixed = new TextDecoder('utf-8', { fatal: false }).decode(bytes);
        if (fixed && !/\uFFFD/.test(fixed)) t = fixed;
      } catch (_) {}
    }
    // 常见实体与错误序列再修一轮
    t = t
  ...
  async function amFetchTextSmart(url) {
    const res = await fetch(url, { cache: 'no-store', credentials: 'omit' });
    if (!res.ok) throw new Error('lyrics fetch ' + res.status);
    const buf = await res.arrayBuffer();
    const tryDec = (label) => {
      try {
    ...
    // 若明显乱码，尝试 GB 系
    if (/\uFFFD/.test(text) || /â.|Ã[©¨ª«]/.test(text)) {
      text = tryDec('gb18030') || tryDec('gbk') || tryDec('latin1') || text;
    }
    return amFixMojibake(text);
  }

  /** 网易云 JSON 逐行：{"t":ms,"c":[{"tx":"..."}]} */
  function amParseNeteaseLyricLine(line) {
    const s = String(line || '').trim();
    if (!s.startsWith('{')) return null;
    try {
      const o = JSON.parse(s);
      if (!o || typeof o !== 'object') return null;
      const parts = Array.isArray(o.c) ? o.c : [];
      const text = parts.map((x) => (x && x.tx != null ? String(x.tx) : '')).join('');
      const tMs = typeof o.t === 'number' ? o.t : 0;
      return { t: tMs / 1000, text: amFixMojibake(text).trim(), meta: true };
    } catch (_) {
      return null;
    }
  }
  ...
  function amExtractArtistFromLyrics(rawText) {
    /* v35: 已停用。作者只从 playlist.json 读取，不再解析 LRC 头标签 */
    return null;
  }

  function amParseLrc(text) {
    const cues = [];
    const raw = amFixMojibake(text);
    String(raw || '')
      .split(/\r?\n/)
      .forEach((line) => {
        ...
        const content = amFixMojibake(line.slice(last)).trim();
        if (times.length && content) {
          times.forEach((t) => cues.push({ t, text: content }));
          return;
        }
        // 跳过空行与 [ti:][ar:] 元信息行（不进滚动）
        if (!content) return;
  ...
  async function amLoadLyrics(track) {
    ...
    if (!track || !track.lyrics) {
      body.innerHTML = '<p class="am-lyrics-empty">暂无歌词</p>';
    ...
    // v35: 作者只认 playlist.json，完全忽略 LRC 头部作词/作曲标签
    const paint = (raw) => {
      ...
      if (cues.length) {
        ...
        body.innerHTML = '<pre class="am-lyrics-plain">' + escapeHtml(fixed) + '</pre>';
      }
      // 立刻按当前进度对齐
      try {
    ...
    try {
      const text = await amFetchTextSmart(track.lyrics);
      paint(text);
    } catch (_) {
  ...
  function amSyncLyrics(time, opts) {
    const cues = amState.lyricsCues;
    if (!cues || !cues.length) return;
    const forceScroll = !!(opts && opts.forceScroll);
    let active = 0;
    const t = Number(time) || 0;
    for (let i = 0; i < cues.length; i++) {
      if (cues[i].t <= t + 0.05) active = i;
      else break;
    }
    const body = document.getElementById('amLyricsBody');
    if (!body) return;
    const lines = body.querySelectorAll('.am-lrc-line');
    ...
    if (!cur) return;
    // 只要歌词面板在 DOM 里可见就滚动（不再强依赖 lyricsOpen 标志）
    const pane = document.getElementById('amLyricsPane');
    const paneVisible = pane && !pane.hidden && pane.offsetParent !== null;
    const bodyVisible = body.offsetParent !== null || body.clientHeight > 0;
    if (!(paneVisible || bodyVisible || amState.lyricsOpen || amState.playerMode)) return;
    if (!(changed || forceScroll)) return;
    try {
      const top = Math.max(0, cur.offsetTop - body.clientHeight / 2 + cur.clientHeight / 2);
      if (typeof body.scrollTo === 'function') {
        body.scrollTo({ top, behavior: forceScroll ? 'auto' : 'smooth' });
      } else {
        body.scrollTop = top;
      }
    } catch (_) {
      try {
        cur.scrollIntoView({ block: 'center', behavior: forceScroll ? 'auto' : 'smooth' });
      } catch (_) {}
    }
  }

  function amDownloadBlob(filename, blob) {
    const a = document.createElement('a');
  ...
  function amSafeFileBase(track) {
    const t = String((track && track.title) || 'track')
      .replace(/[\\/:*?"<>|]+/g, '_')
      .trim() || 'track';
    return t.slice(0, 80);
  }

  async function amDownloadTrackAudio(track) {
    if (!track || !track.src) return;
    const base = amSafeFileBase(track);
    try {
      const res = await fetch(track.src, { mode: 'cors', credentials: 'omit', cache: 'no-store' });
      ...
      if (ct.includes('flac')) ext = '.flac';
        ...
        const m = String(track.src).match(/\.(flac|mp3|m4a|ogg|wav|aac)(?:\?|$)/i);
        if (m) ext = '.' + m[1].toLowerCase();
      }
      amDownloadBlob(base + ext, blob);
    } catch (_) {
      // CORS 失败时直接打开源链，由浏览器处理下载/新标签
      const a = document.createElement('a');
      a.href = track.src;
      a.target = '_blank';
      a.rel = 'noopener';
  ...
  async function amDownloadTrackLyrics(track) {
    if (!track) return;
    const base = amSafeFileBase(track);
    let text = amState._lyricsRaw || '';
    if (!text && track.lyrics && !/^(https?:\/\/|\/)/i.test(track.lyrics)) {
      text = track.lyrics;
    }
    if (!text && track.lyrics && /^(https?:\/\/|\/)/i.test(track.lyrics)) {
      try {
        text = await amFetchTextSmart(track.lyrics);
      } catch (_) {}
    }
    if (!text) {
      alert('当前曲目没有可下载的歌词');
    ...
    amDownloadBlob(base + '.lrc', blob);
  }

  function amHideCtxMenu() {
    const m = document.getElementById('amCtxMenu');
    if (m) m.hidden = true;
  }

  function amShowCtxMenu(x, y, track) {
    // 已并入站点 site-context-menu；保留空实现避免旧调用报错
    if (track) amState.currentTrack = track;
  }
  function amHideCtxMenu() {
    const m = document.getElementById('amCtxMenu');
  ...
  function amSetPlayerMode(on) {
    ...
    if (on) {
      ...
      const track = amGetPlayingTrack();
      if (track) {
        const idx = amFindIndexInTracks(track);
        if (idx >= 0) amState.index = idx;
        amSyncNowPlaying();
        amLoadLyrics(track);
      }
      amUpdateHeroForContext();
  ...
  function amSetSeekLoading(on) {
    const track = document.getElementById('amSeekTrack');
    const fill = document.getElementById('amSeekFill');
    if (!track) return;
    if (on) {
  ...
  function amLocatePlaying() {
    ...
    const table = document.querySelector('.am-track-table');
    if (!list) return;
    let row = list.querySelector('.am-track-row.is-active');
    if (!row) {
      const track = amGetPlayingTrack && amGetPlayingTrack();
      const idx = track ? amFindIndexInTracks(track) : amState.index;
      if (idx >= 0) row = list.querySelector('.am-track-row[data-index="' + idx + '"]');
    }
    if (!row) return;
    const scroller = table || list;
    try {
  ...
  function amPaintVolumeBar() {
    const vol = document.getElementById('amVolume');
    const audio = document.getElementById('amAudio');
    if (!vol) return;
    let v = Number(vol.value);
    if (!Number.isFinite(v)) v = audio ? Number(audio.volume) : 0.85;
    ...
    vol.style.setProperty('background-image', grad, 'important');
  }
  ...
  function amToggle() {
    ...
    if (!track) {
      if (amState.tracks.length) amPlayAt(0);
      return;
    }
    if (!audio) return;
    if (audio.paused) {
      if (!audio.src && track.src) {
        amPlayAt(amState.index);
        return;
      }
      audio.play().then(() => { amState.playing = true; amSyncNowPlaying(); }).catch(() => {});
    } else {
      audio.pause();
      amState.playing = false;
      amSyncNowPlaying();
  ...
  function amBindPipControls(root) {
    ...
    root.querySelectorAll('[data-act]').forEach(function (btn) {
      btn.addEventListener('click', function (e) {
        ...
        if (act === 'prev' && typeof amNext === 'function') amNext(-1);
        else if (act === 'next' && typeof amNext === 'function') amNext(1);
        else if (act === 'toggle' && typeof amToggle === 'function') amToggle();
        else if (act === 'back10' && audio) audio.currentTime = Math.max(0, (audio.currentTime || 0) - 10);
        else if (act === 'fwd10' && audio && audio.duration) {
  ...
  function amRefreshPipToggleIcon(root) {
    if (!root) return;
    var audio = document.getElementById('amAudio');
  ...
  function amBuildPipShell(cover) {
    ...
    return (
      '<div class="am-pip-square">' +
        bg +
        '<div class="am-pip-fg">' +
          '<div class="am-pip-float-head">' +
            '<button type="button" class="am-pip-float-close" aria-label="关闭"><i class="fas fa-times"></i></button>' +
            '<div class="am-pip-float-title"></div>' +
            '<div class="am-pip-float-artist"></div>' +
          '</div>' +
  ...
  function amPipApplyStyle(doc) {
    ...
    style.textContent =
      ...
      '.am-pip-float-head{position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;padding:14px 36px 8px;user-select:none;text-align:center;}' +
      '.am-pip-float-title{width:100%;font-size:17px;font-weight:650;line-height:1.3;opacity:.98;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;text-align:center;}' +
      '.am-pip-float-artist{width:100%;font-size:12px;font-weight:400;line-height:1.25;opacity:.72;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;text-align:center;}' +
      '.am-pip-float-close{position:absolute;top:8px;right:8px;border:0;background:transparent;color:#fff;cursor:pointer;padding:4px;display:inline-flex;opacity:.85;}' +
      '.am-pip-float-body{flex:1;overflow:auto;padding:8px 12px;scrollbar-width:thin;scrollbar-color:rgba(255,255,255,.28) transparent;}' +
      '.am-pip-float-body::-webkit-scrollbar{width:6px;background:transparent;}' +
      '.am-pip-float-body::-webkit-scrollbar-track{background:transparent;}' +
      '.am-pip-float-body::-webkit-scrollbar-thumb{background:rgba(255,255,255,.28);border-radius:99px;border:none;}' +
      '.am-pip-float-body::-webkit-scrollbar-thumb:hover{background:rgba(255,255,255,.4);}' +
      '.am-lrc-line{padding:4px 6px;border-radius:6px;opacity:.75;cursor:pointer;text-align:center;}' +
      '.am-lrc-line:hover{background:rgba(255,255,255,.08);opacity:.95;}' +
  ...
  function amBindPipLyricClicks(body) {
    if (!body) return;
    body.querySelectorAll('.am-lrc-line').forEach(function (line) {
      line.style.cursor = 'pointer';
      line.addEventListener('click', function (e) {
        e.preventDefault();
        e.stopPropagation();
        var t = Number(line.getAttribute('data-t'));
        var a = document.getElementById('amAudio');
        if (!a || !Number.isFinite(t)) return;
        try {
          window.__amSeeking = true;
          amState.audioLoading = false;
          try { amSetSeekLoading(false); } catch (_) {}
          var fill = document.getElementById('amSeekFill');
          var d = a.duration;
          if (fill && d && isFinite(d) && d > 0) {
            fill.classList.remove('is-loading-pulse');
            fill.style.width = (Math.max(0, Math.min(1, t / d)) * 100) + '%';
          ...
          a.currentTime = Math.max(0, t);
          if (a.paused && amState.playing) a.play().catch(function () {});
          try { amSyncLyrics(t, { forceScroll: true }); } catch (_) {}
          try { amSyncPipContent({ smooth: true }); } catch (_) {}
          setTimeout(function () { window.__amSeeking = false; }, 120);
        } catch (_) {
          window.__amSeeking = false;
        }
      });
  ...
  function amSyncPipContent(opts) {
    ...
    roots.forEach(function (root) {
      try {
        ...
        var artistEl = root.querySelector('.am-pip-float-artist');
        if (artistEl) artistEl.textContent = (track && track.artist) ? track.artist : '—';
  ...
  async function amOpenPipLyrics() {
    ...
    try {
      if (window.documentPictureInPicture && window.documentPictureInPicture.requestWindow) {
        ...
        var _ae = doc.body.querySelector('.am-pip-float-artist');
        if (_ae) _ae.textContent = (track && track.artist) ? track.artist : '—';
    ...
    var _ae2 = float.querySelector('.am-pip-float-artist');
    if (_ae2) _ae2.textContent = (track && track.artist) ? track.artist : '—';
    ...
    amBindPipControls(float);
    amRefreshPipToggleIcon(float);
    var head = float.querySelector('.am-pip-float-head');
    var dx = 0, dy = 0, ox = 0, oy = 0, drag = false;
...
function amNext(delta) {
    ...
    if (mode === 'loop') {
      const audio = document.getElementById('amAudio');
      if (audio) { try { audio.currentTime = 0; audio.play().catch(function(){}); } catch (_) {} }
      return;
    }
    ...
    // order
    const track = amGetPlayingTrack();
    const cur = amFindIndexInTracks(track);
    const base = cur >= 0 ? cur : (amState.index >= 0 ? amState.index : 0);
    const next = (base + d + amState.tracks.length * 50) % amState.tracks.length;
    amPlayAt(next);
  }
  ...
  function setupAmPlayerOnce() {
    ...
    amSyncPlayModeUi();
    const modeBtn = document.getElementById('amPlayModeBtn');
    if (modeBtn && !modeBtn.dataset.bound) {
      modeBtn.dataset.bound = '1';
      modeBtn.addEventListener('click', (e) => { e.preventDefault(); amCyclePlayMode(); });
    }
    const amAudio = document.getElementById('amAudio');
    ...
    document.querySelectorAll('.am-track-head [data-sort]').forEach((el) => {
      ...
      el.addEventListener('click', () => amSetSort(el.getAttribute('data-sort')));
    });
    // 播放器内随处右键 → 当前曲下载
    const playerRoot = document.getElementById('amPlayer');
    if (playerRoot && !playerRoot.dataset.ctxGlobal) {
      playerRoot.dataset.ctxGlobal = '1';
      /* player contextmenu uses site-context-menu */
    }
    ...
    if (playAll) playAll.addEventListener('click', () => {
      ...
      if (!amState.tracks.length) return;
      let idx = 0;
      const pick = amState.heroPickTrack;
      if (pick) {
        const found = amFindIndexInTracks(pick);
        if (found >= 0) idx = found;
      }
      if ((amState.playMode || 'shuffle') === 'shuffle') {
    ...
    if (prev) prev.addEventListener('click', () => amNext(-1));
    const next = document.getElementById('amNextBtn');
    if (next) next.addEventListener('click', () => amNext(1));
    const vol = document.getElementById('amVolume');
    const audio = document.getElementById('amAudio');
    if (vol && audio) {
      audio.volume = Number(vol.value) || 0.85;
      const paintVol = () => { try { amPaintVolumeBar(); } catch (_) {} };
      paintVol();
      const onVol = () => {
    ...
    if (audio) {
      ...
      const dlAudioBtn = document.getElementById('amDownloadAudioBtn');
      const dlLrcBtn = document.getElementById('amDownloadLrcBtn');
      if (dlAudioBtn && !dlAudioBtn.dataset.bound) {
        dlAudioBtn.dataset.bound = '1';
        dlAudioBtn.addEventListener('click', () => {
          const tr = amState.index >= 0 ? amState.tracks[amState.index] : null;
          if (tr) amDownloadTrackAudio(tr);
        });
      }
      if (dlLrcBtn && !dlLrcBtn.dataset.bound) {
        dlLrcBtn.dataset.bound = '1';
        dlLrcBtn.addEventListener('click', () => {
          const tr = amState.index >= 0 ? amState.tracks[amState.index] : null;
          if (tr) amDownloadTrackLyrics(tr);
        });
      }
      const npBar = document.getElementById('amNowPlaying');
      if (npBar && !npBar.dataset.ctxBound) {
        npBar.dataset.ctxBound = '1';
        npBar.addEventListener('contextmenu', (e) => {
          e.preventDefault();
          const tr = amState.index >= 0 ? amState.tracks[amState.index] : null;
          if (tr) amShowCtxMenu(e.clientX, e.clientY, tr);
        });
  ...
  async function fetchAmCatalog() {
    let lastErr = null;
    for (const url of AM_CATALOG_URLS) {
      try {
        const res = await fetch(url, { cache: 'no-store', credentials: 'omit' });
        if (!res.ok) { lastErr = new Error(url + ' ' + res.status); continue; }
        const data = await res.json();
        const lists = amParseCatalog(data);
    ...
    console.warn('[playlist] 加载失败', lastErr);
    return [];
  }

  async function loadPlaylistRoute(force = false) {
    if (routeLoadState.playlist && !force) return;
    routeLoadState.playlist = true;
    setupAmPlayerOnce();
    const listEl = document.getElementById('amTrackList');
    if (listEl) listEl.innerHTML = '<div class="am-track-empty">加载曲目中…</div>';
  ...
  document.addEventListener('click', function(e) {
    ...
    if (e.button === 0 && !isPointerDown && performance.now() >= suppressEffectsUntil) {
      if (!(window.__suppressClickEffects || document.body.classList.contains('is-slider-dragging')) && !shouldSkipClickEffects(e.target)) {
...
function saveImageAs(img) {
  ...
  function buildContextMenuItems(target) {
    ...
    // 博客内嵌音频 + 我的歌单 Apple Music 播放器
    const playerEl = target && target.closest
      ? target.closest('.glass-audio-player, .md-audio-player, .md-lyrics, .md-song-block, #amPlayer, .am-player')
      : null;
    if (playerEl) {
      const isAm = !!(playerEl.id === 'amPlayer' || playerEl.classList.contains('am-player') || playerEl.closest('#amPlayer'));
      if (isAm) {
        const tr = (typeof amGetPlayingTrack === 'function' ? amGetPlayingTrack() : null)
          || (typeof amState !== 'undefined' && amState.index >= 0 ? amState.tracks[amState.index] : null);
        const audioSrc = tr && (tr.src || tr.audio) || '';
        const lyricsSrc = tr && (tr.lyrics || tr.lrc) || '';
        items.push({ type: 'sep' });
        items.push({
          id: 'dl-music',
          icon: 'fa-download',
          ...
          action: () => {
            if (!audioSrc) return;
            const name = decodeURIComponent((audioSrc.split('?')[0].split('/').pop() || 'audio')) + '';
        ...
        items.push({
          ...
          action: () => {
            if (!lyricsSrc) return;
            const name = decodeURIComponent((String(lyricsSrc).split('?')[0].split('/').pop() || 'lyrics.lrc')) + '';
  ...
  document.addEventListener('pointerdown', function(e) {
    const t = e.target;
    if (t && (t.matches('input[type=range], .am-seek-track, #amSeekTrack, #amVolume') || t.closest('.am-seek-track, .am-np-volume, #amSeekTrack'))) {
      document.body.classList.add('is-slider-dragging');
      window.__suppressClickEffects = true;
    }
  }, true);
  document.addEventListener('pointerup', function() {
    document.body.classList.remove('is-slider-dragging');
    setTimeout(function(){ window.__suppressClickEffects = false; }, 80);
  }, true);
